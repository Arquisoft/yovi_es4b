...
* **Users**: The back-end service responsible for user management, built with Node.js.
...
Gateway --> Users : HTTP JSON (REST) - management API
Gateway --> Gamey : HTTP JSON (REST)
Auth --> UserDB : TCP (DB driver) - auth data
Users --> UserDB : TCP (DB driver) - user management
...
| User management, admin APIs, metrics endpoint
...
| HTTP (REST) for auth endpoints (`/register`, `/login`, `/me`) / TCP (MongoDB driver)
